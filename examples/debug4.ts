/**
 * Debug script - match exact sonar structure
 */
...
async function main() {
  ...
  // 2. Large ForwardOpen matching sonar structure exactly
  ...
  // With length prefix: 02 20 06 24 01
  const requestPath = new Uint8Array([0x02, 0x20, 0x06, 0x24, 0x01]);

  // Connection path to Message Router (class 0x02, instance 0x01)
  // With length prefix: 02 20 02 24 01
  ...
  console.log(`  Service: 0x54 (Large ForwardOpen)`);
  console.log(`  Request path: ${toHex(requestPath)} (CM class 0x06, inst 0x01)`);
  console.log(`  FO params: ${forwardOpenParams.length} bytes`);
  console.log(`  Connection path: ${toHex(connectionPath)} (MR class 0x02, inst 0x01)`);
  console.log(`  Total CIP message: ${cipMessage.length} bytes`);
